...
export default {
  ...
  methods: {
    ...
    axios
      ...
      .then((response) => {
        ...
        this.SuratKeluar = response.data.sort((a, b) => {
    ...
    async HapusSurat(id) {
      let result = await axios.delete("http://localhost:3004/SuratKeluar/" + id);
      console.warn(result);
  ...
  const filename = `Buku agenda masuk bulan ${this.selectedMonth}-${this.selectedYear}.pdf`;
